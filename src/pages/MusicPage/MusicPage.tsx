...
export const MusicPage: React.FC = () => {
  const [albums, setAlbums] = useState<IAlbum[]>(null);
  ...
  return (
    <div className="p-4 max-w-7xl	m-auto">
      {albums &&
        <div div className="grid grid-cols-[repeat(auto-fit,_minmax(12rem,_1fr))] auto-rows-[8rem] gap-x-6 gap-y-6">
          {albums.map(album => {
            return (
              <>
              ...
              {
                album.tracks.items.map(track => <Track key={track.id} {...track} images={album.images} />)
              }
              </>
            )
          }
